...
public final class ProductsManager {
    ...
    public boolean executeScript(String path) {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(path))) {
            ...
            while (lines.size() > 0) {
                try {
                    executeSuccessful = doCommand(lines, br);
    ...
    private boolean doCommand(List<String> lines, BufferedReader br) throws IOException {
        ...
        if (command == Command.ADD || command == Command.UPDATE) {
            ...
            return addUpdateProductFromScript(command, params, command == Command.ADD
                    ? null : Integer.parseInt(lines.get(1)));
        } else if (command == Command.EXECUTE_SCRIPT) {
            if (scripts.contains(lines.get(1))) {
                return false;
            } else {
                return executeScript(lines.get(1));
            }
        } else {
